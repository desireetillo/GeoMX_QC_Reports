---
title: "GeoMX DSP Transcriptome Atlas Initial QC for `r params$output_prefix`"
...
We are running this workflow with FlaggedQCRemoval set to `r RemoveFlagSegment`.  If this is TRUE we will now remove samples that did not pass our QC thresholds.

```{r RemoveFlagged}

if(RemoveFlagSegment){
  print("Subsetting our dataset has removed samples which did not pass QC")
  myData <- myData[, QCResults$QCStatus == "PASS"]
}
if(!RemoveFlagSegment){
  print("We are NOT removing flagged segments")
}
dim(myData)
...
fig <- plot_ly(pData(target_myData),
               ...
               colors='Dark2',
               autosize = F, width = 700, height = 700,margin=m)
